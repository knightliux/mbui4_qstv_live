...
public class ProgramCache {
	...
	public static boolean isExist(Context ctx){
		logger.i("Program cache file = " + Configs.getCacheFile(ctx));
		File file = new File(Configs.getCacheFile(ctx));
		return file.exists();
	}
	
	public static boolean isDirectory(Context ctx){
		File file = new File(Configs.getCacheFile(ctx));
		return file.isDirectory();
	}
	
	public static boolean delFile(Context ctx){
		File file = new File(Configs.getCacheFile(ctx));
		if(file.exists())
			return file.delete();
		return false;
	...
	public static void save(String saveGsonStr,Context ctx){
		File file = new File(Configs.getCacheFile(ctx));
	...
	public static String getGsonString(Context ctx){
		File file = new File(Configs.getCacheFile(ctx));
		if(file.isDirectory()) return "";
		BufferedReader br = null;
		try {
			br = new BufferedReader(new FileReader(Configs.getCacheFile(ctx)));
			String resultStr = "";
	...
	public static boolean checkIsSame(String gsonStr,Context ctx){
		File file = new File(Configs.getCacheFile(ctx));
